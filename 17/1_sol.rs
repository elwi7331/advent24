...
impl<'a> Machine<'a> {
    fn new(a: usize, b: usize, c: usize, program: &'a [u8]) -> Self {
        Self { reg_a: a, reg_b: b, reg_c: c, program: program, pc: 0 }
    }
    ...
    fn out(&mut self, arg: u8) -> u8 {
        self.pc += 2;
        (self.combo_to_int(arg).unwrap() % 8) as u8
    }
    ...
    fn run(&mut self) -> Vec<u8> {
        ...
        while self.pc < self.program.len() {
            match self.program[self.pc] {
                ...
                4 => self.bxc(),
                5 => {res.push(self.out(self.program[self.pc+1]));},
...
fn main() {
    for a in 35_184_372_088_832.. { // 35_184_372_088_832 smaller than 281_474_976_710_656
        let input = vec![2,4,1,5,7,5,1,6,0,3,4,0,5,5,3,0];
